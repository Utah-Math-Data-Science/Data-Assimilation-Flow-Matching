    ...
    def __iter__(self):
        if self.model.cfg.train_on_initial_predicted_state:
            ...
            if self.model.cfg.resample_initial_predicted_state:
                for done, sample_time_step, time, sampled_state in self.model.sampling_steps(predicted_state, next_observation, self.dataset.observe):
                    if self.model.cfg.epoch_count_sampling > 0 and not done:
                        yield self.model.cfg.epoch_count_sampling, sample_time_step, time, sampled_state, next_observation, True
        ...
        for time_step, t_now_and_next, predicted_state, next_observation, ignore_observation in tqdm(
           ...
           desc='Estimating state at time step',
            ...
            if not ignore_observation or self.model.cfg.resample_predicted_state_when_ignoring_observation:
                for done, sample_time_step, time, sampled_state in self.model.sampling_steps(next_predicted_state, next_observation, self.dataset.observe):
                    if self.model.cfg.epoch_count_sampling > 0 and not done:
                        yield self.model.cfg.epoch_count_sampling, sample_time_step, time, sampled_state, next_observation, ignore_observation
            else:
