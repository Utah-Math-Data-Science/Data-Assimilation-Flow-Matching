...
def _enkf_pert_obs_analysis(
    ...
    if torch_noise_generator is None:
        obs_perturbations = sigma_y * torch.tensor(
            rng.standard_normal(size=(N_ensemble, d_obs)),
            dtype=ensemble_f.dtype,
            device=ensemble_f.device,
        )
    else:
        obs_perturbations = sigma_y * torch.randn(
            (N_ensemble, d_obs),
            dtype=ensemble_f.dtype,
            device=ensemble_f.device,
            generator=torch_noise_generator,
        )
...
def ensemble_kalman_filter_analysis(
    rng,
    ensemble_f, observation_y, observation_operator_ens, R_obs=None, sigma_y=None,
    ...
    R_obs_factor=None, torch_noise_generator=None ):
